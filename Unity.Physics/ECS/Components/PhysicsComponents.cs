...
namespace Unity.Physics
{
    ...
    public struct PhysicsVelocity : IComponentData
    {
        ...
        {
            var currentTransform = new RigidTransform(bodyOrientation.Value, bodyPosition.Value);
            var com = new float4(bodyMass.Transform.pos, 1f);
            var velocity = new PhysicsVelocity
            {
                Linear = (math.mul(targetTransform, com) - math.mul(currentTransform, com)).xyz * stepFrequency
            };
            velocity.SetAngularVelocityWorldSpace(
                bodyMass, bodyOrientation,
                math.mul(targetTransform.rot, math.inverse(bodyOrientation.Value)).ToEulerAngles() * stepFrequency
            );
            return velocity;
        }
    }
