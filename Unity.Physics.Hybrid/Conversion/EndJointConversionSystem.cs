...
namespace Unity.Physics.Authoring
{
    ...
    public partial class EndJointConversionSystem : GameObjectConversionSystem
    {
        ...
        static readonly ComponentTypeSet k_JointComponentsSingle = new ComponentTypeSet(
            typeof(PhysicsConstrainedBodyPair),
            typeof(PhysicsJoint)
        );
        static readonly ComponentTypeSet k_JointComponentsMultiple = new ComponentTypeSet(
            typeof(PhysicsConstrainedBodyPair),
            typeof(PhysicsJoint),
        ...
        /// <summary>
        /// Create a new entity for a joint associated with the specified authoring component. Other
        /// systems can later find this joint by using <see cref="GetJointEntities"/>. Use this method
        /// when the behavior specified by the authoring component can be described with a single <see cref="PhysicsJoint"/>
        /// (see also <seealso cref="CreateJointEntities"/>). The new entity will have all required
        /// component types as well as a readable name in the Entity Debugger.
        /// </summary>
        ///
        /// <param name="authoringComponent">   An authoring component being converted. </param>
        /// <param name="constrainedBodyPair"> A component describing the bodies constrained by the
        /// joint. </param>
        /// <param name="joint">               The reference frames and set of constraints to apply to
        /// the bodies. </param>
        ///
        ...
        /// Other systems can later find these joints by using <see cref="GetJointEntities"/>. Use this
        /// method instead of <see cref="CreateJointEntity"/> when the behavior specified by the
        /// authoring component requires multiple <see cref="PhysicsJoint"/> components to describe. The
        /// new entities will have all required component types, <see cref="PhysiscsJointCompanion"/>
        /// buffers, as well as readable names in the Entity Debugger.
        /// </summary>
        ///
        /// <param name="authoringComponent">   An authoring component being converted. </param>
        /// <param name="constrainedBodyPair"> A component describing the bodies constrained by the
        /// joints. </param>
        /// <param name="joints">              The set of reference frames and corresponding constraints
        /// to apply to the bodies. </param>
        /// <param name="newJointEntities">    (Optional) An optional list to populate with all of the
        /// new entities. </param>
        ...
        )
        {
            ...
            var baseName = $"{nameEntityA} + {nameEntityB}";
#endif
            for (var i = 0; i < joints.Length; ++i)
            {
                uint worldIndex = DstEntityManager.GetSharedComponent<PhysicsWorldIndex>(constrainedBodyPair.EntityA).Value;
                Assertions.Assert.AreEqual(
                    worldIndex,
                    constrainedBodyPair.EntityB == Entity.Null ? worldIndex : DstEntityManager.GetSharedComponent<PhysicsWorldIndex>(constrainedBodyPair.EntityB).Value);

                var jointEntity = CreateAdditionalEntity(authoringComponent);

                DstEntityManager.AddSharedComponent(jointEntity, new PhysicsWorldIndex(worldIndex));

#if UNITY_EDITOR
                var nameFs = new FixedString64Bytes();
                FixedStringMethods.CopyFromTruncated(ref nameFs, $"{baseName} ({joints[i].JointType})");
                DstEntityManager.SetName(jointEntity, nameFs);
#endif

                DstEntityManager.AddComponent(
        ...
        /// <summary>
        /// Get all joint entities that have been produced by the specified authoring component. In order
        /// to work, these entities must have been produced by either <see cref="CreateJointEntity"/> or <see cref="CreateJointEntities"/>
        /// .
        /// </summary>
        ///
        /// <param name="authoringComponent"> The authoring component that produced one or more joint
        /// entities. </param>
